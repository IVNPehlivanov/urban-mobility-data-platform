...
spark_home = "/Users/user/Desktop/spark_and_hadop/spark-3.5.4-bin-hadoop3" # Change to own hadoop-folder. Use double slashes when needed. You can also use os.sep instead of the double slaches.
hadoop_home = "/Users/user/Desktop/spark_and_hadop/hadoop-3.4.1-src" # For windows: use the hadoop folder in bigdatatools. For linux/mac: Try with provided hadoop folder in bigdatatools, if that doesn't work, download hadoop from the apache website: https://hadoop.apache.org/release/3.4.0.html
java_home = os.popen("/usr/libexec/java_home -v 17").read().strip()
print("Dynamically set JAVA_HOME:", java_home)#########################
